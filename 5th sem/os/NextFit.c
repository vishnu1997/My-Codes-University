...
int main(){
    ...
    while(1){
        ...
        if(ch==1){
            ...
            scanf("%d",&mm);
            flag=-1;                                 //checks if any memory is less or not
            flag2=0;                                 //this to prevent infinte circular looping
            for(;i<n;i=(i+1)%n,flag2++){             //circular rotation
                ...
                if(flag2==n){                       //if it has checked through all the spaces but couldn't find any
                    break;
                }
            }
